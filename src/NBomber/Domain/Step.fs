...
module RunningStep =
    ...
    let tryExec (dep: StepDep) (step: RunningStep) = task {
        try
        ...
        with
        | ex ->
            dep.Logger.Fatal(ex, $"Step with index '{step.StepIndex}' from Scenario: '{dep.ScenarioInfo.ScenarioName}' has failed")
            let response = Response.fail(statusCode = Constants.StepInternalClientErrorCode,
                                         error = $"step internal client exception, stepIndex: {step.StepIndex}, error: {ex.Message}")
            let resp = { StepIndex = step.StepIndex; ClientResponse = response; EndTimeMs = 0; LatencyMs = 0 }
            dep.ScenarioStatsActor.Publish(AddResponse resp)
            return ValueSome response
    }

    let execSteps (dep: StepDep) (steps: RunningStep[]) (stepsOrder: int[]) = task {
        ...
        | Some execControl -> do! execCustomExec(dep, steps, execControl)
        | None             -> do! execRegularExec(dep, steps, stepsOrder)
    }
